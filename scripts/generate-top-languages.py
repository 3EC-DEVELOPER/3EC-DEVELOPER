...
DEVICON_DIR = Path(os.environ.get("DEVICON_DIR", "assets/devicons"))
...
LANGUAGE_META = {
    ...
    "C++": {
        "color": "#8A2BFF",
        "icon_color": "#00599C",
        "devicon": "cplusplus-original.svg",
    ...
    return [
        {
            ...
            "devicon": LANGUAGE_META.get(name, {}).get("devicon"),
...
def devicon_fragment(filename):
    path = DEVICON_DIR / filename
    if not path.exists():
        return None
    svg = path.read_text(encoding="utf-8").strip()
    match = re.search(r"<svg\b([^>]*)>(.*)</svg>", svg, flags=re.DOTALL | re.IGNORECASE)
    if not match:
        return None
    ...
    if len(view_box) != 4 or view_box[2] <= 0 or view_box[3] <= 0:
        return None
    return {"view_box": view_box, "inner": inner}
